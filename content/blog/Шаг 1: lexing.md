...
Создадим структуру, которая будет содержать всю логику лексера. `Source` -- входные символы, которые подаются на вход.
...
Добавим вывод токенов при итерировании по `Lex`.
...
В коде используется функция `take_while`, которая собирает в буффер все символы, которые попадают под правило, передаваемое как аргумент.
...
Надеюсь статья была интересной, а самое главное полезной. Если что-то непонятно -- пишите в комменты или мне в личные сообщения, чтобы я мог вам это объяснить.
